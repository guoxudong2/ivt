...
class OnlyIVTransformerMultiInputBlock(nn.Module):
    def __init__(self, hidden, attn_heads, head_size, feed_forward_hidden, dropout, attn_dropout,
        ...
        self.disable_cross_attention = disable_cross_attention
        self.isolate_subnetwork = isolate_subnetwork
        self.self_attention_chemo_iv = MultiHeadedAttention(num_heads=attn_heads, d_model=hidden,
                                                            head_size=head_size,
                                                            dropout=attn_dropout,
                                                            positional_encoding_k=self_positional_encoding_k,
                                                            positional_encoding_v=self_positional_encoding_v,
                                                            final_layer=final_layer)
        self.self_attention_radio_iv = MultiHeadedAttention(num_heads=attn_heads, d_model=hidden,
                                                            head_size=head_size,
                                                            dropout=attn_dropout,
                                                            positional_encoding_k=self_positional_encoding_k,
                                                            positional_encoding_v=self_positional_encoding_v,
    ...
    def forward(self, x_tov, active_entries_treat_outcomes, active_entries_vitals=None):
        ...
        x_chemo_iv_ = self.self_attention_chemo_iv(x_chemo_iv, x_chemo_iv, x_chemo_iv,
                                                   self_att_mask_chemoiv_radioiv,
                                                   True) if x_chemo_iv is not None else None
        x_radio_iv_ = self.self_attention_radio_iv(x_radio_iv, x_radio_iv, x_radio_iv,
                                                   self_att_mask_chemoiv_radioiv,
                                                   True) if x_radio_iv is not None else None

...
class IvSyntheticCancerDataset(Dataset):
    """
    ...
    def __init__(self,
                 ...
                 u_ratio: float = 0,
                 chemo_iv_ratio: float = 0,
                 radio_iv_ratio: float = 0):
        """
        Args:
        ...
        if mode == 'factual':
            ...
            self.data = simulate_counterfactuals_treatment_seq(self.params, seq_length, projection_horizon, cf_seq_mode, u_ratio, chemo_iv_ratio, radio_iv_ratio)
    ...
    def process_sequential_test(self, projection_horizon, encoder_r=None, save_encoder_r=False):
        ...
        if not self.processed_sequential:
            ...
            for i in range(num_patient_points):
                ...
                seq2seq_U[i] = U[i, fact_length: fact_length + projection_horizon, :]
                seq2seq_chemo_iv[i] = chemo_iv[i, fact_length: fact_length + projection_horizon, :]
                seq2seq_radio_iv[i] = radio_iv[i, fact_length: fact_length + projection_horizon, :]
    ...
    def process_autoregressive_test(self, encoder_r, encoder_outputs, projection_horizon, save_encoder_r=False):
        ...
        if not self.processed_autoregressive:
            ...
            for i in range(num_patient_points):
                ...
                current_dataset['U'][i] = U[i, fact_length:fact_length + projection_horizon, :]
                current_dataset['chemo_iv'][i] = chemo_iv[i, fact_length:fact_length + projection_horizon, :]
                current_dataset['radio_iv'][i] = radio_iv[i, fact_length:fact_length + projection_horizon, :]
...
class IvSyntheticCancerDatasetCollection(SyntheticDatasetCollection):
    """
    ...
    def __init__(self,
                 ...
                 num_patients: dict,
                 u_ratio: float,
                 chemo_iv_ratio: float,
                 radio_iv_ratio: float,
                 seed=100,
                 window_size=15,
                 max_seq_length=60,
        ...
        Args:
            ...
            treatment_mode: multiclass / multilabel
        """
        super(IvSyntheticCancerDatasetCollection, self).__init__()
        self.seed = seed
        np.random.seed(seed)
